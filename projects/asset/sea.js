...
(function(seajs){
    ...
    var splitComboUrl = function(url){
        ...
        result.host = host
        result.files = []
        for(var i= 0,len = urls.length;i<len;i++){ result.files.push(urls[i])="" }="" return="" result="" **="" *="" warning:="" rewrite="" this="" function="" to="" fit="" your="" combo="" file="" structure="" default:="" split="" by="" define(function(){})="" @param="" code="" var="" splitcombo="(data.localcache" &&="" data.localcache.splitcombo)="" ||="" function(code,="" url,="" files){="" arr="code.split('define')" for(var="" i="0,len" =="" arr.length;i<len;i++){="" if(arr[i]){="" result.push('define'+arr[i])="" fetchinglist="{}" onload="function(url){" mods="fetchingList[url]" delete="" fetchinglist[url]="" while="" ((m="mods.shift()))" m.load()="" module.prototype.fetch="function(requestCache){" mod="this" seajs.emit('fetch',mod)="" url="mod.requestUri" mod.uri="" iscombourl="isCombo(url)" if(fetchinglist[url]){="" fetchinglist[url].push(mod)="" fallback="function(url){" fetch.call(mod,requestcache)="" if(!iscombourl="" remotemanifest[url]){="" in="" version="" control="" cached="storage.get(url)" cachedvalidated="validate(url," cached)="" if(remotemanifest[url]="=" localmanifest[url]="" cachedvalidated){="" is="" ready="" go="" if(!use(url,="" cached)){="" fallback(url)="" }else{="" onload(url)="" otherwise,="" get="" latest="" from="" network="" fetchajax(url="" +="" '?v="+Math.random().toString(), function(resp){
                    if(resp && validate(url, resp)){
                        if(!use(url, resp)){
                            fallback(url)
                        }else{
                            localManifest[url] = remoteManifest[url]
                            storage.set(" manifest',="" json.stringify(localmanifest))="" update="" one="" storage.set(url,="" resp)="" })="" }else="" if(iscombourl){="" try="" find="" available="" cache="" splited="splitComboUrl(url)," needfetchajax="false" -="" 1;i="">=0;i--){
            ...
            fetchAjax(comboUrl + '?v='+Math.random().toString(), function(resp){
                ...
                if(splited.files.length == splitedCode.length){
                    ...
                    for(var i= 0,len = splited.files.length;i</len;i++){></$.executing&&(b.onload(),b.exec()),b.exports},u.base=o,u.dir=o,u.cwd=l,u.charset="utf-8",t.config=function(a){for(var></$.saved&&(c.id=b.id||a,c.dependencies=b.deps||[],c.factory=b.factory,c.status=$.saved,b("save",c))},s.get=function(a,b){return></$.fetching?e.fetch(g):e.status===$.saved&&e.load();for(var></$.loaded?e._waitings[a.uri]=(e._waitings[a.uri]||0)+1:a._remain--;if(0===a._remain)return>