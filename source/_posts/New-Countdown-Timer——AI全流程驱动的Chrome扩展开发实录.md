...
金芽红茶取自茶株芽头，通常一杯金芽的最佳饮用周期是前六泡，第 1-2 泡茶质未开，汤色偏白，醇浓中略带茶涩，仿佛人之青年；第 3-4 泡茶质舒展，汤色微红，口感香甜醇厚，仿佛人至中年，这也是一杯金芽的最佳饮用时期；第 5-6 泡茶质疏软，茶涩尽褪，口感柔滑中不失爽口，仿佛人之暮年。要得到金芽的最佳口感需要严格把控冲泡时间，以为 85 度水为宜，第一泡冲泡 7 秒，第二泡 14 秒，第三泡 30 秒，第四泡 1 分钟，第五泡 2 分钟，第六泡 5 分钟。因为金芽太过细嫩，一旦冲泡过久容易损伤茶叶内质，茶汤会浓浊酸涩，而且整杯茶的生命周期也被破坏，如果还想好好的喝杯茶，就只能换茶重来。
...
大致看一遍，首先需要准备的是 logo 素材和提示音文件。

设计 logo 还是 ChatGPT/Sora 最擅长，于是打开 ChatGPT：

```
设计一个用于倒计时的logo，logo 的字母 c 是 countdown 的首字母，选择深绿色为主色调，因为作者的主要使用场景是泡茶的时候倒计时，防止长时间冲泡茶汤太浓。
```
...
这个图距离正式可用还差一个圆角，于是我下意识的打开了 Photoshop，接下来我要画一个圆角矩形路径、将路径转换成选区、反选、删除选区、保存，好麻烦，于是我关掉了 Photoshop，找了一个在线处理图片圆角的网站，顺便还能转换尺寸，点击下鼠标搞定。

声音素材就简单了，下载一个放到指定目录就行。
...
然后就是吭哧吭哧的`开发=>运行=>报错=>开发……`

到这一步全部调试完成后，我得到了一个具备倒计时基础功能的 chrome 扩展，然后可以开始注入我的核心需求了：

```
更新需求文档，增加自定义定时器存储功能，可以让用户预先设置一系列定时器，每次启动插件展示定时器列表，可以快捷的启动列表中的一个定时器，每个定时器除了可以设置不同时间外还可以设置不同的声音和颜色，以便于区分
```

这一步 AI 给出的结果是不可用的，没有通过需求反推出需要增加的界面以及界面间新的交互逻辑，即便 Claude 3.7 在项目达到一定完成度后，继续拔高式的提需求就会明显感觉理解力不足，其他模型就更不用提了，只能用做提词器，距离 agent 差很远。
...
类似这种高度依赖 AI 完成的项目过去我还做过几个，比如写智能合约，写后端，这些都是我不具备的技术栈，我只能依赖 AI 去完成。可能在外行眼里 AI 确实很神奇，仿佛一夜之间工程师失去了存在的必要。但我作为从业十年多的 web 开发者，却越来越觉得 AI 只是一种很好用的工具，像过去那些好用的工具一样，AI 能加速完成使用者能力范围内的事，但它并不神奇，借助 AI 也无法让人突破能力边界。

以自身为例，虽然从没开发过 chrome 扩展，但我掌握 chrome 扩展所需要的核心技术栈，凭借多年开发经验完全可以很快学会，如果没有 AI 我可能会花很多时间在实践中踩坑，AI 只是加速了这个注定成功的过程。

实际上即便不掌握具体技术栈，只要知道系统整体的技术架构就足以驾驭 AI 做项目了，比如我开发过的智能合约和电商后端项目，到项目结束我也仍然不会写 solidity 和 graphql，但不妨碍我完成了项目需求。而开发过程中真正受限的是什么呢，反而是自身的能力。比如说不具备金融知识就不可能写好 DIFI 合约，这种知识是 AI 不能给到你的，这就是边界，需要自己去突破。

凡事都有代价，AI 带来便利的代价是什么呢？是停止成长，工程师会失去在实践中积累、沉淀的机会，从而阻断量变引发质变这个成长必要的进程。从这个角度看 AI 确实很像魔法，只不过是黑魔法，有所收获，也有所献祭。
