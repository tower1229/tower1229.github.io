---
title: "Nodejs爬虫实践小记"
description: "Nodejs将前端开发语言移植到服务器端，如今一个前端开发者使用Nodejs很容易就能实现一个网络爬虫，这在之前是不可想象的，本文介绍一个简单的Nodejs爬虫的开发过程，只想看代码的直接拉到最后。"
pubDate: 2017-06-09T05:24:22.000Z
tags: ["Nodejs","爬虫"]
---
...
![cnBeta 新闻页相邻文章链接由 JavaScript 动态生成](/asset/QQ%E6%88%AA%E5%9B%BE20170609154821.png)

这是常见的防抓取措施，通过异步请求获取关联页面链接，再由js动态生成，查看网络面板可以看到页面确实发出了一个异步请求，结果中有我们要的关联页面ID:

![浏览器网络面板中 cnBeta 异步请求返回关联页面 ID](/asset/QQ%E6%88%AA%E5%9B%BE20170609154855.png)

接下来分析这个请求，可以发现有两个参数，想必都可以从HTML中找到：

![cnBeta 异步请求的两个参数分析](/asset/QQ%E6%88%AA%E5%9B%BE20170609154920.png)

第一个参数`_csrf`很容易在源代码中直接搜索到了：

![在 HTML 源码中搜索到的 _csrf 参数](/asset/QQ%E6%88%AA%E5%9B%BE20170609154946.png)

第二个参数全文检索找不到：

![第二个请求参数在全文检索中未找到](/asset/QQ%E6%88%AA%E5%9B%BE20170609155011.png)

观察这个参数的结构，发现数据被两个逗号隔成三段，于是猜测数据是由于多部分拼接而来的，分开检索果然找到了：

![拆分检索后找到的参数后两段数据](/asset/QQ%E6%88%AA%E5%9B%BE20170609155046.png)
